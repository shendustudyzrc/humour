...
public class UserServiceImp implements IUserService{
	...
	public boolean updatePassword(User user) throws SQLException {
		...
		SqlSession session=null;
		session=sqlSessionFactory.openSession();
		int n=session.update("com.mybatis.UserMapper.updatePassword",user);
		if(n>0){
			b=true;
		}
		session.commit();
	...
	public User selectUserByUsername(String username) throws SQLException {
		...
		SqlSession session=null;
		session=sqlSessionFactory.openSession();
		user=session.selectOne("com.mybatis.UserMapper.selectUserByUsername",username);
		session.commit();
		return user;
	}
	...
	public boolean updateUserByUsername(User user) throws SQLException {
		...
		SqlSession session=null;
		session=sqlSessionFactory.openSession();
		int n=session.update("com.mybatis.UserMapper.updateUserByUsername",user);
		if(n>0){
			b=true;
		}
		session.commit();
	...
	public boolean updateImageByUsername(User user) throws SQLException {
		...
		SqlSession session=null;
		session=sqlSessionFactory.openSession();
		int n=session.update("com.mybatis.UserMapper.updateImageByUsername",user);
		if(n>0){
			b=true;
		}
		session.commit();
